...
- `adr-0006-mcp-token-resolution.md` — Resolución de tokens MCP + `.env.mcp` portable + upgrade de herramientas + self-update + activación en el kit maestro (Opción C híbrida: plantilla con tokens/`{env:...}`, resolución runtime incondicional e idempotente en `Ensure-OpenCodeMcp`, `environment`/ruta real como mecanismo efectivo, flags `-ForceUpgradeTools`/`-SkipSync`, 12 guardrails). **Aceptado 2026-09-24**.
...
- `specs/007-mcp-token-resolution/` — **Feature**: Resolución de tokens MCP (`__*_CMD__`/`{env:...}`) incondicional e idempotente + `.env.mcp` por proyecto (gitignored) + `-ForceUpgradeTools` + `Update-Self` + modo kit seguro `-SkipSync` + guard del sync sobre `opencode.json` + degradación con WARN. Contiene `spec.md`, `plan.md`, `research.md`, `data-model.md`, `analyze.md`, `converge.md`, `tasks.md`, `qa-report.md`. Estado: **IMPLEMENTADA + QA APTO (2026-09-24)** — 46/46 tareas; kit maestro operativo (3 MCPs `enabled: true`). Artefactos: `arquitectura/adr/adr-0006-mcp-token-resolution.md`, `seguridad/007-mcp-token-resolution-threat-model.md`. ADR-0006.
...
| MCP | URL | Estado | Uso |
|-----|-----|--------|-----|
| `context-mode` | https://github.com/mksglu/context-mode | 🟢 Instalado (2026-09-12) | Optimiza ventana de contexto: indexación FTS5+BM25 de docs (`ctx_index`, `ctx_search`, `ctx_fetch_and_index`), ejecución sandbox (`ctx_execute`), continuidad de sesión, mantenimiento (`ctx_purge`, `ctx_stats`, `ctx_upgrade`, `ctx_doctor`). Instalado: `npm install -g context-mode` (v1.0.169). Registrado en `.vscode/mcp.json` + hooks `.github/hooks/context-mode.json`. **MCP activo siempre** (ruta global resuelta en opencode.json, 2026-09-23). Licencia **ELv2** (source-available, no MIT). Requiere Node >= 22.5. Guía: `MCPs/context-mode.md` |
| `codebase-memory-mcp` | https://github.com/DeusData/codebase-memory-mcp | 🟢 Instalado (2026-09-12) | Grafo de conocimiento del código (`index_repository`, `query`, `semantic_search`). Paso 2 del pipeline del ecosistema. Instalado: `npm install -g codebase-memory-mcp` (v0.9.0). Registrado en `.vscode/mcp.json`. **MCP activo siempre** (ruta global resuelta en opencode.json, 2026-09-23). Usar `--mode fast` (evita crash con proyect_ext). Licencia **MIT** ✅ (verificada 2026-09-12). Guía: `MCPs/codebase-memory-mcp.md` |
