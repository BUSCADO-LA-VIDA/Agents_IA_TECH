...
| MCP | `context-mode` | v1.0.169 | � Instalado (2026-09-12) | Optimiza ventana de contexto: indexación FTS5+BM25 (`ctx_index`, `ctx_search`, `ctx_fetch_and_index`), ejecución sandbox (`ctx_execute`), agrupación (`ctx_batch_execute`), mantenimiento (`ctx_purge`, `ctx_stats`, `ctx_upgrade`, `ctx_doctor`). Licencia ELv2. Requiere Node >= 22.5. Instalado: `npm install -g context-mode`. Registrado en `.vscode/mcp.json` + hooks `.github/hooks/context-mode.json`. Guía: `MCPs/context-mode.md` |
...
| MCP `context-mode` | Instalado | ✅ Sí (v1.0.169) | OK — instalado y registrado en `.vscode/mcp.json` + hooks `.github/hooks/context-mode.json` |
...
3. ✅ Instalar y configurar `context-mode` (2026-09-12) — instalado (v1.0.169), registrado en `.vscode/mcp.json` + hooks `.github/hooks/context-mode.json`
