...
- [ ] `[RELOCATE]` **Script nuevo `relocate-apps-to-src.ps1`: reubicación a `src\AppXXX` con confirmación obligatoria**
  ...
  - **Fase documental**:
    - `documentador`: Nuevo RF en la spec (script standalone + punto futuro de integración) + README (sección de reubicación) + ADR-0003 (RNF-04 vs opt-in confirmado).
    - `security-auditor`: Riesgos (mover código equivocado, venvs en uso, corte a mitad de movido, confirmación fatigada, rollback).
  - **Fase implementación**:
    ...
    - `qa-senior`: Tests DryRun + fixtures en `$env:TEMP` (S/N/T/C, no-interactivo no mueve, venv reportado, estructura preservada, bootstrap intacto).
