...
| 2026-09-12 | **Usar MCPs como herramienta primaria de consulta (ahorrar tokens)**: Los MCPs (`context-mode`, `codebase-memory-mcp`, `markitdown`) deben usarse como herramienta primaria para consultar documentación/código en lugar de leer archivos directos cuando sea posible. Leer directo gasta más tokens. Los MCPs están instalados pero deben activarse como herramientas en la sesión. | workflow |
| 2026-09-12 | **Re-indexación forzada en la primera consulta (sin IA)**: En la primera consulta de cada sesión se fuerza la re-indexación de los MCPs (`context-mode`, `codebase-memory-mcp`) y la actualización de `analisis-memoria.md` antes de que la IA consulte por MCP. La re-indexación es procesamiento local sin IA (FTS5/grafo, determinista y barato). Si un MCP no está instalado → instalarlo (con confirmación) y repetir la re-indexación. Flujo: instalar → re-indexar → consultar. | workflow |
| 2026-09-12 | **`"type": "stdio"` obligatorio en MCPs locales**: Todo MCP local configurado en `.vscode/mcp.json` debe declarar explícitamente `"type": "stdio"` (transporte por defecto para MCPs que se lanzan como proceso hijo vía `command`). Aplica a `context-mode`, `codebase-memory-mcp` y `markitdown`. Incluirlo siempre al configurar MCPs en cualquier proyecto para que quede listo al instalar. | setup |
