...
| **tokenslayer-mcp-server** | https://github.com/ajvikram/TokenSlayer | **MIT** ✅ (verificada 2026-09-18, fase documental `[TOKENSLAYER]`) | Servidor MCP standalone (carpeta `mcp-server/`, clonar + compilar con Node v24.14.0) + extensión VS Code `ajvikram.tokenslayer` v1.5.0. Compactación de contexto: esqueletos AST + call graphs + patch estructural. Herramientas: `analyze_files`, `analyze_workspace`, `analyze_dependency_chain`, `expand_node`, `apply_patch`, `get_stats`, `clear_stats`. Complementa a `context-mode` (docs) y `codebase-memory-mcp` (grafo), no los reemplaza. Guía: `Documentacion/Agents_IA_TECH/MCPs/tokenslayer.md`. |
| **Agents_IA_TECH (repo maestro)** | https://github.com/BUSCADO-LA-VIDA/Agents_IA_TECH | (kit transversal) | **Repo maestro del ecosistema**: fuente del kit transversal que `Sync-TransversalKit` (función de `plataformador-bootstrap.ps1`) clona en modo shallow y copia en cada proyecto (`.github/`, `.opencode/`, `.doc_agents/`, `.specify/memory/constitution.md`, `AGENTS.md`, `opencode.json`, `README.md`, `sync-agents.ps1`). Definido por el usuario (2026-09-17) en el ADR-0003. **NUNCA** sincroniza `Documentacion/<AppName>/` de las apps. |
