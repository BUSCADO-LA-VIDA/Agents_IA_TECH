...
| **RF-19** | **Instalar Graphify con árbol de decisión MCP-preferido (`Configure-Graphify` real, refina RF-09)** | Tarea `[GRAPHIFY-INSTALL]` (plan aprobado 2026-09-19; decisión explícita del usuario: instalarlo, y **si expone MCP, preferir la vía MCP**). **Investigación `documentador` 2026-09-19 (vía web, sin clones)**: (1) la URL del manifest `https://github.com/tomasgraph/graphify` devuelve **404** (no existe / no pública); el upstream real es **`https://github.com/Graphify-Labs/graphify`** (paquete PyPI **`graphifyy`**, licencia MIT según `LICENSE-MIT`; cabecera del README dual Apache-2.0/MIT — pendiente de confirmación de `security-auditor`); (2) **SÍ expone MCP server** (embebido, sin carpeta `mcp-server/`): `python -m graphify.serve graphify-out/graph.json` (stdio; también `--transport http`) con herramientas `query_graph`, `get_node`, `get_neighbors`, `shortest_path`, `list_prs`, `get_pr_impact`, `triage_prs` (requiere extra `mcp`: `uv tool install "graphifyy[mcp]"`); (3) **NO hay binario `bin/graphify` ni `lib/`** upstream (es paquete Python, no binario Go): instalación con `uv tool install graphifyy` (recomendado) / `pipx install graphifyy` / `pip install graphifyy` (requiere Python 3.10+), skill con `graphify install --platform opencode`; verificación CLI con `graphify --version` / `graphify --help`. **Árbol de decisión (NO cambia)**: **Rama A (preferida, viable)**: registrar el MCP stdio en `opencode.json` (`type: local`, `command: [python, -m, graphify.serve, <repo>/graphify-out/graph.json]`, `enabled: true`) + `.vscode/mcp.json` (`type: stdio`), como tokenslayer, tras construir el grafo (`graphify <path>` → `graphify-out/graph.json`); verificar handshake (`tools/list` responde las 7 herramientas). **Rama B (fallback, forma adaptada)**: la copia binaria `bin/graphify → .opencode/bin/` + `lib/ → .opencode/lib/graphify/` **no aplica tal cual** (esos paths no existen upstream); el fallback real es instalación como herramienta Python (mismo patrón que `markitdown`: vía `uv`/`pip`, sin copiar código al kit) + verificación `graphify --version`. En ambos casos: pin de versión en `dependencias-manifest.yml`, **WARN (no fallo duro)** si no se puede instalar, **`-DryRun` informa**, logs sin secrets. Firma actual de `Configure-Graphify` (`param([string]$RootPath = "")`, `$DryRun` de scope script, estilo `Write-*`, solo avisa si falta — ver `scripts/plataformador-bootstrap.ps1` L2036-2069) intacta. **Gaps para `devops`/`security-auditor`** (fuera de alcance documental, este rol no toca manifest ni scripts): la URL del manifest está muerta (404) y el owner real (`Graphify-Labs`) **no está en la allowlist** del bootstrap (fail-closed bloquearía la descarga) → `devops` corrige el manifest solo con decisión explícita del usuario; `security-auditor` revisa supply chain del paquete PyPI + riesgos del MCP server (mismo patrón que tokenslayer). |
...
15. **Graphify con árbol MCP-preferido (RF-19, `[GRAPHIFY-INSTALL]`)**: tras el bootstrap, Graphify queda instalado por **Rama A** (MCP stdio `graphify.serve` registrado en `opencode.json` + `.vscode/mcp.json`, handshake `tools/list` OK con sus herramientas, grafo `graphify-out/graph.json` construido) o, si no hay grafo/MCP disponible, por **Rama B** (CLI `graphify --version` responde tras `uv`/`pip install graphifyy`); si no se puede instalar, avisa WARN y no falla; con `-DryRun` solo informa qué haría; versión fijada en `dependencias-manifest.yml`; firma/estilo de `Configure-Graphify` intactos.
