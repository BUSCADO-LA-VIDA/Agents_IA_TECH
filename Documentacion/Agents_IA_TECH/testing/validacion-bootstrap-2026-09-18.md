...
| Símbolo | Significado |
|---------|-------------|
| `<project-root>` | Ruta raíz de cualquier proyecto (ej. `<tu-proyecto>`, `$env:PROJECT_ROOT`). |
| `<kit-root>` | Ruta donde está instalado el kit transversal (puede coincidir con `<project-root>`). |
| `AppXXX` | App del proyecto en modo real, anonimizada (evidencia histórica). |
| `App1..AppN` | Conjunto de apps descubiertas dinámicamente por el bootstrap (raíz o `src/*`). |
| `[S]` / `[N]` / `[T]` / `[C]` | Sí / No / Todos / Cancelar (confirmaciones interactivas). |
| PASS / FAIL / WARN | Resultado de un criterio o comando. |
| `H-N` | Hallazgo reportado por QA (no corregido, por restricción de paths). |

## Comandos ejecutados (todos en `pwsh` 7.6.6 salvo indicación)

1. `git status --porcelain` (ANTES) — `<project-root>`
2. `scripts/plataformador-bootstrap.ps1 -DryRun -NoRestart -SkipInstall -SkipIndexing` (raíz del kit) → EXIT 0
3. `git status --porcelain` (DESPUÉS) → idéntico al ANTES
4. `sync-agents.ps1 -DryRun` → EXIT 0; `git status` idéntico
5. `scripts/plataformador-bootstrap.ps1 -DryRun -NoRestart -SkipInstall -SkipIndexing -ProjectRoot <project-root>` → EXIT 0
6. `npx ecc-agentshield scan -p <project-root>` → EXIT 0, Grade A (98/100)
...
## T-I6 + T-V1 (DryRun raíz del kit) — PASS
...
- Salida verificada:
  ...
  - (f) **PASS** — 5 WARN de apps ausentes (App1..AppN del proyecto), sin clonar (RNF-06, esperado)
  - (g) **PASS** — `proyect_ext/spec-kit queda en la raíz (no se mueve)`; existe en disco (`.devcontainer`, `.github`, `.specify`, `bundles`, `docs`, …)
  - (h) **PASS** — `Graphify disponible en: <kit-root>/.opencode/lib/graphify`
...
## T-V1 2ª parte (DryRun sobre `<project-root>` externo) — PASS

- El parámetro `-ProjectRoot` **sí existe** (línea 27 del script); se usó `-ProjectRoot <project-root>` → EXIT 0.
- `<project-root>` **no es repo git** (`fatal: not a git repository`), por lo que la verificación es por salida, no por diff:
  - Las `App1..AppN` **sí existen en raíz** → `se respeta su ubicación, RNF-04; no se mueve nada` + solo líneas `DryRun: aseguraría Documentacion/<app>/`.
  - Sync declara `NUNCA tocaría Documentacion/<AppName>/`; ninguna línea de la salida escribe en `Documentacion/` real (todo `DryRun:`).
  - `proyect_ext/spec-kit queda en la raíz` OK; Graphify OK (`<project-root>/.opencode/lib/graphify`).
  - `Documentacion/` del proyecto intacta (directorios listados sin cambios: `agents`, `Agents_IA_TECH`, `bitacoras`, `ecosistema`, `App1`, `App2`).
...
- Extra: `context-mode doctor` (invocado por el propio bootstrap en paso 10) → PASS general; 2 WARN no bloqueantes:
  - `Legacy MCP registration` (`mcp.context-mode` redundante con el plugin).
  - `npm (MCP): local vunknown, latest v1.0.169` (actualización disponible).

---

## T-V4 (security scan) — PASS

- `npx ecc-agentshield scan -p <project-root>` → **Grade A (98/100)**, 22 archivos, 3 hallazgos: 0 critical, 0 high, 2 medium, 1 low. EXIT 0.
...
2. Primer modo real con `-DryRun` previo + `git status` antes/después, empezando por proyecto de prueba (`<project-root>` ya validado en DryRun).
